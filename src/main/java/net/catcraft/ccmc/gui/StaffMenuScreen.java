...
public final class StaffMenuScreen extends Screen {
    ...
    private StaffMenuScreen(Screen oldScreen, View view) {
        super(Component.literal("CatCraft Staff Menu"));
    ...
    @Override protected void init() {
        ...
        if (view == View.ROOT) {
            ...
            add(x, y, w, "Chat Channels", () -> show(View.CHAT)); y += 24;
            add(x, y, w, "Settings", () -> ClientScreens.show(CcmcSettingsScreen.create(this))); y += 24;
        ...
        if (view == View.TOOLS) {
            ...
            if (rank().ordinal() >= StaffRank.SENIOR_MODERATOR.ordinal()) {
    ...
    private void prefill(String value, String label) {
        if (!ChatInputPrefill.prefill(this, value)) local("[CatCraft Staff] couldn't prepare " + label + " in chat; no command was sent.");
    }
